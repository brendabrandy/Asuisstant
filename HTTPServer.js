...
dispatcher.onGet("/activity",function(req,res){
	var client = new pg.Client(conString);
	client.connect(function(err) {
		if(err) {
			return console.error('could not connect to postgres', err);
		}
		client.query('SELECT * FROM public.activity', function(err, result) {
			if(err) {
			  return console.error('error running query', err);
			}
			res.writeHead(200, {'Content-Type': 'application/json'});
...
dispatcher.onPost("/activity_write",function(req,res){
	var querydata = url.parse(req.body, true).query;
	var client = new pg.Client(conString);
	var sql = SQL.insert('public.activity').values(querydata).done();
	var sql_script="";
	for(var i = 0, len = sql.length; i<len;i++){
		if(sql[i] == '\"' ){
			sql_script = sql_script + '\'';
